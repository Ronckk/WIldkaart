...
(function(){
  ...
  function findPlaces(q){
    ...
    return starts.concat(contains).slice(0,8).map(function(b){
      return { key:b.id, label:b.n, name:nameHtml(b.n), meta:'Laatste melding: '+fmtDate(b.ev[0].d)+' &middot; '+(b.ev[0].diersoort ? WDK.esc(b.ev[0].diersoort)+' &middot; ' : '')+TYPE_LABEL[b.ev[0].ty] };
    });
  }
